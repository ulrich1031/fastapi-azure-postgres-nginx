...
class GPTResearcher:
    ...
    def __init__(
        ...
        self.vector_retreiver_cfg = vector_retreiver_cfg
    ...
    async def conduct_rag(self):
        if not self.vector_retreiver_cfg:
            return []
        contexts = await self.__get_context_from_index__(
            vector_retreiver_cfg=self.vector_retreiver_cfg,
            query=self.query,
            cfg=self.cfg,
        )
        return contexts
    ...
    async def write_report_yield(self, existing_headers: list = []):
        report = ""
        ...
        async for response in generate_report_yield(
            query=self.query,
    ...
    async def __get_rag_search_queries__(self, query: str, count: int = 5):
        from app.utils.openai.azureopenai_client import AzureOpenAIClient

        azureopenai_client = AzureOpenAIClient()
        result = json.loads(
            await azureopenai_client.ainvoke(
                response_format={"type": "json_object"},
                messages=[
                    {
                        "role": "user",
                        "content": f"""
                    I'm going to generate a report for these query.
                    ------------------------------------------
                    {query}
                    ------------------------------------------
                    I want you to generate {count} search queries that can be used to reterieve documents to answer the user query.
                    You should output in JSON format.
                    
                    Example:
                    {{
                        "queries": ["John and Jane Doe contributions to STEM Education Programs", "Specific STEM Education Programs funded by John and Jane Doe donations", "Outcomes achieved through donations by John and Jane Doe to STEM Education Programs"]
                    }}
    ...
    async def __get_context_from_index__(
        self, vector_retreiver_cfg: dict, query: str, cfg: Config
    ):
        """
        Get context from vector indexes.

        Parameters:

            vector_retreiver_cfg (dict): Configs for retreivers.

            query (str): query to fetch

            cfg (dict): Researcher configuration
        """
        vector_retriever_class = get_vector_retriever(type=cfg.vector_retreiver)
        vector_retriever = vector_retriever_class(**vector_retreiver_cfg)
        search_queries = await self.__get_rag_search_queries__(query)
        contexts = []
            ...
            contexts.append(
                {
                    "contexts": [
                        context.__dict__
                        for context in await vector_retriever.ainvoke(input=query)
                    ],
                    "query": query,
                }
            )
    ...
    async def process_web_search_query(self, query: str, max_results: 5):
        return await self.__process_sub_query(query, [], max_results)
    
    async def __process_sub_query(self, sub_query: str, scraped_data: list = [], max_results=5):
        ...
        content = await self.__get_similar_content_by_query(sub_query, scraped_data, max_results=max_results)
        ...
        return {"contexts": content, "query": sub_query}
    ...
    async def __get_similar_content_by_query(self, query, pages, max_results=5):
        if self.verbose:
            await stream_output(
                "logs",
